...
class GrafikPendidikanController extends Controller
{
    public function index()
    {
        // Mengambil data yang sudah digroup dan dihitung total kerjasama berdasarkan tahun dan semester
        $data = KerjasamaTridharmaPendidikan::join('tahun_ajaran_semester', 'kerjasama_tridharma_pendidikan.tahun_ajaran_id', '=', 'tahun_ajaran_semester.id')
            ->select(
        ...
        // Kirim data ke view
        return view('pages.admin.petugas.grafik.kerjasama-tridharma.pendidikan.index', compact('data'));
    }
}

...
class KesesuaianKerjaController extends Controller
{
    ...
    {
        try {
            $EvalKesesuaianKerjaMhs = new EvalKesesuaianKerja();
            ...
            return view('pages.admin.kinerja-lulusan.evaluasi-lulusan.kesesuaian-kerja.form', [
                'kesesuaian_kerja' => $EvalKesesuaianKerjaMhs,
                'tahun_ajaran' => $tahunAjaran,
                'tahun' => $tahun,
                'form_title' => 'Tambah Data',
                'form_action' => route('admin.kinerja-lulusan.evaluasi-lulusan.kesesuaian-kerja.store', $tahunAjaran),
                'form_method' => "POST",
            ]);
        } catch (\Exception $e) {
            return back()->withErrors($e->getMessage());
    ...
    public function store(Request $request,string $tahunAjaran)
    {
        try {
            ...
            $create = EvalKesesuaianKerja::create($validated);
            if ($create) {
                return redirect()->route('admin.kinerja-lulusan.evaluasi-lulusan.kesesuaian-kerja.index', $tahunAjaran)
    ...
    {
        try {
            $EvalKesesuaianKerjaMhs = EvalKesesuaianKerja::with('user')->first();
            $tahunAjaranObj = TahunAjaranSemester::where('slug', $tahunAjaran)->firstOrFail();
            $tahunAjaranId = $tahunAjaranObj->id;
            $tahun = $tahunAjaranObj->tahun_ajaran;
            ...
            return view('pages.admin.kinerja-lulusan.evaluasi-lulusan.kesesuaian-kerja.form', [
                ...
                'form_action' => route('admin.kinerja-lulusan.evaluasi-lulusan.kesesuaian-kerja.update', [
                    'tahunAjaran' => $tahunAjaran,
                    'kesesuaianId' => $EvalKesesuaianKerjaMhs->id,
                ]),
                'form_method' => "PUT",
            ]);
        } catch (\Exception $e) {
    ...
    {
        try {
            $EvalKesesuaianKerjaMhs = IpkLulusan::findOrFail($id);
            $delete = $EvalKesesuaianKerjaMhs->delete();
